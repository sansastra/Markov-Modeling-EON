function [stateOccupancyPattern,transitionForRoutesForClasses,blockingStates,possibleStatesTransitionForClasses]= FirstFit(WoSC,totalNumberOfSlots,bandwidthPerClass,routes,nr_ksp)
                               ...
                               %for i=1: valid
                                   ...
                                   for link=1:numberOfLinks %occupancy of all links forms a vector                                   
                                       if(routes{ksp}(odPair,link)==0)
                                           ...
                                           stateOccupancyTemp(link,:)=stateOccupancyPattern{link}(loop,:);
                                           stateOccupancyTemp(link,index1(1))=(ksp-1)*numberOfODpairs +odPair; % this is the route number 
                                           stateOccupancyTemp(link,index1(1)+1:index1(1)+bandwidthPerClass(c)-1)=remainingBand;
                                       end
                                   end
        ...
        for loop =begin : oldLength
            for odPair=1:numberOfODpairs 
                for c =1: classes  
                           ...
                           if(routeLength(ksp,odPair)==1)
                              ...
                              valid=1;
                               ...
                               if(length(index1_local)==routeLength(ksp,odPair)) % % request is allocated without continuity
                                   ...
                                   index1(validLinks{ksp,odPair}(1,1),1)= index1_local{1}(1,1);
                                   index1(validLinks{ksp,odPair}(1,2),1)= index1_local{2}(1,1);  
                                   blockingStates(loop,odPair,c)=0;
                    ...
                    for i=1: valid % here valid can be 0 or 1
                        stateOccupancyTemp=zeros(numberOfLinks,totalNumberOfSlots);
                        for link=1:numberOfLinks %occupancy of all links forms a vector                                   
                            if(routes{ksp}(odPair,link)==0)
                                stateOccupancyTemp(link,:)=stateOccupancyPattern{link}(loop,:);
                                ...
                                stateOccupancyTemp(link,:)=stateOccupancyPattern{link}(loop,:);
                                stateOccupancyTemp(link,index1(link,i))=(ksp-1)*numberOfODpairs +odPair;
