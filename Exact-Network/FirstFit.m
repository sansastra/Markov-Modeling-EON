function [stateOccupancyPattern,transitionForRoutesForClasses,blockingStates,possibleStatesTransitionForClasses]= FirstFit(WoSC,totalNumberOfSlots,bandwidthPerClass,routes)
            ...
            for odPair=1:numberOfRoutes             
                for c =1: classes
                    ...
                    for link=2:routeLength(odPair)
                        ...
                        tempOccupancy=all(tempOccupancy==0,1);
                        tempOccupancy=~tempOccupancy;
                        b = diff([0 tempOccupancy==0 0]);
                       max_res = find(b==-1) - find(b==1);
                       if ((isempty(max_res)) | (max_res<bandwidthPerClass(c))) % | => logical or
                           ...
                           for link=1:numberOfLinks %occupancy of all links forms a vector                                   
                               if(routes(odPair,link)==0)
                                   ...
                                   stateOccupancyTemp(link,:)=stateOccupancyPattern{link}(loop,:);
                                   stateOccupancyTemp(link,index1(1))=odPair;
                           ...
                           if(routeLength(odPair)==1)
                              ...
                              valid=1;
                               ...
                               if(length(index1_local)==routeLength(odPair)) % % request is allocated without continuity
                                   ...
                                   index1(validLinks{odPair}(1,1),1)= index1_local{1}(1,1);
                                   index1(validLinks{odPair}(1,2),1)= index1_local{2}(1,1);  
                                   blockingStates(loop,odPair,c)=0;
